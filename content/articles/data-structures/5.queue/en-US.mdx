...
<QueueVisualization title="enqueue(99) on [3, 8, 17, 42]:" items={[3, 8, 17, 42]} incoming={99}>
...
<QueueVisualization title="dequeue() from [3, 8, 17, 42]:" items={[3, 8, 17, 42]} outgoing>
...
<Callout type="info">
This builds on the [Linked Lists article](/categories/data-structures/articles/linked-lists): enqueue appends at the tail (O(1) because we track the tail pointer), dequeue removes from the head (O(1) because we track the head pointer). A queue is a linked list with access restricted to exactly these two ends.
</Callout>
