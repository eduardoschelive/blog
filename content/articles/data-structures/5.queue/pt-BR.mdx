...
slug: queue
coverImage: queue-cover.svg
thumbnail: queue-thumbnail.svg
keywords:
  [
    queue,
    FIFO,
...
<QueueVisualization title="enqueue(99) em [3, 8, 17, 42]:" items={[3, 8, 17, 42]} incoming={99}>
...
<QueueVisualization title="dequeue() de [3, 8, 17, 42]:" items={[3, 8, 17, 42]} outgoing>
...
<Callout type="info">
Isso se baseia no [artigo sobre Linked Lists](/categorias/estruturas-de-dados/artigos/linked-lists): enqueue adiciona no tail (O(1) porque rastreamos o pointer tail), dequeue remove do head (O(1) porque rastreamos o pointer head). Uma queue é uma linked list com acesso restrito exatamente a essas duas extremidades.
</Callout>
